...
impl AstarStats {
    ...
    pub fn values(&self) -> (Vec<String>, Vec<String>) {
        [
            ...
            self.format_avg('>', 8, "A*", 1000. * self.timing.astar),
            self.format_avg('>', 8, "h()", 1000. * self.h.h_duration),
            self.format_avg('>', 8, "prune", 1000. * self.h.prune_duration),
            self.format_avg('>', 8, "reorders", 1000. * self.timing.reordering),
