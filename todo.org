...
* Semi-global
- use A*PA2 instead of A*PA
- Drop support for inexact matches
  - Replace(d) by local pruning.
  - Think about the exact differences between the two.
  - /Bottom-up hirschberg/: merge adjacent matches into longer ones
- Find inexact matches to save first step of local pruning?
