...
** TODO Reduce number of retries by adding an offset to the BucketQueue that's updated after every prune.
This will require some manual updates after pruning, but removes the ~n lg n~
