...
** TODO Fix consistency of pruning with shadow points
...
** TODO A*:
*** TODO Instead of storing f for expanded states, store g for queue states
Only process if f is up-to-date and g_queue == g_expanded
*** TODO Optimize for matching states: process these directly instead of pushing & popping from the queue
