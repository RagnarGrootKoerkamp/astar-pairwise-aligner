...
** TODO Pruning may result in double expansion of matches
...
** TODO GappedSeedHeuristic differs from SeedHeuristic in some edge cases
** TODO Do we need some special consideration for seeds exactly
Potentially the same as above. There used to be code for this but it was not
re-implemented for Contours.
** TODO NaiveContours sometimes drops values by more than max_len.
Partially fixed by replacing the pruning of the single root point by also
