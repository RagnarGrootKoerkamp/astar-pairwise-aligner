...
use super::diagonal_transition::{Direction, GapVariant};
use super::nw::PATH;
...
pub struct DiamondTransition<CostModel> {
...
impl<const N: usize> DiamondTransition<AffineCost<N>> {
    ...
    fn next_front(
        ...
        let d0 = a.len() as i32 - b.len() as i32;
        // Get the front `cost` before the last one.
        let get_front = |cost, d: i32| {
            println!("AAA {s}\t{cost}\t{d}\t");
            &prev[(s as i32 - cost as i32 + d + if d < d0 { d0 - d } else { 0 }) as usize]
        };
                ...
                for d in next.range().clone() {
                    ...
                    for layer_idx in 0..N {
                        ...
                        let affine_f = match cm.affine_type {
                            InsertLayer => max(
                                // Gap open
                                get_front(cm.open + cm.extend, d + 1).m()[d + 1] + 1,
                                // Gap extend
                                get_front(cm.extend, d + 1).affine(layer_idx)[d + 1] + 1,
                            ),
                            DeleteLayer => max(
                                // Gap open
                                get_front(cm.open + cm.extend, d - 1).m()[d - 1] + 1,
                    ...
                    if let Some(cost) = self.cm.sub {
                        f = max(f, get_front(cost, d).m()[d] + 2);
                    }
                    // Insertion
                    if let Some(cost) = self.cm.ins {
                        f = max(f, get_front(cost, d + 1).m()[d + 1] + 1);
                    }
                    // Deletion
                    ...
                    for idx in 0..N {
                        ...
                        match cm.affine_type {
                            InsertLayer | DeleteLayer => {
                                // Gap close
                                f = max(f, get_front(cm.open + cm.extend, d).m()[d])
                            }
                            _ => todo!(),
                        };
                    }
                ...
                for d in next.range().clone() {
                    ...
                    for idx in 0..N {
                        ...
                        next.affine_mut(idx)[d] = match cm.affine_type {
                            ...
                            InsertLayer => max(
                                next.m()[d + 1] + 1,
                                get_front(cm.extend, d + 1).affine(idx)[d + 1] + 1,
                            ),
                            // max(Gap open, Gap extend)
                            DeleteLayer => max(
                                next.m()[d - 1] + 1,
...
impl<const N: usize> Aligner for DiamondTransition<AffineCost<N>> {
    fn visualize(
        &self,
        a: &Sequence,
        b: &Sequence,
        ...
        loop {
            let mut next = Front::new_with_buffer(
                Fr::MIN,
                (d0 - s as i32)..=(s as i32),
                // self.d_range(s),
                self.left_buffer,
                self.right_buffer,
            );
            if self.next_front(a, b, fronts, &mut next, v, s) {
                // FIXME: Reconstruct path.
                return ((s as i32 - d0) as u32, vec![], Cigar::default());
            }
