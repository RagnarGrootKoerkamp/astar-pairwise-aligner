...
pub struct AffineBruteForceContours {
    valued_arrows: Vec<(Arrow, Cost)>,
}

impl Contours for AffineBruteForceContours {
    fn new(arrows: impl IntoIterator<Item = Arrow>, _max_len: I) -> Self {
        let mut this = AffineBruteForceContours {
            valued_arrows: Vec::default(),
        };
        for arrow in arrows {
            let val = this.score(arrow.end + Pos(0, 0)) + arrow.score as Cost;
    ...
    fn score(&self, q: Pos) -> Cost {
        self.valued_arrows
            .iter()
            .filter(|(arrow, _)| {
                if q.diag() == arrow.start.diag() {
                    q <= arrow.start
                } else {
                    q + Pos(0, 0) <= arrow.start
                }
            })
