...
fn preserve_match(a: Seq, b: Seq, seeds: &Seeds, m: &Match, p: usize) -> bool {
    let s = m.start;
    ...
    for g in 1..pd as Cost {
        ...
        for d in d_range.clone() {
            next[d - 1] = max(next[d - 1], fr[d]);
            next[d] = max(next[d], fr[d] + 1);
            next[d + 1] = max(next[d + 1], fr[d] + 1);
        }
        swap(&mut fr, &mut next);
...
pub fn local_pruning(a: Seq, b: Seq, matches: &mut Matches, p: usize) {
    ...
    matches
        .matches
        .retain(|m| preserve_match(a, b, &matches.seeds, m, p));
