...
** Installation
First, [[https://rustup.rs/][install rustup]], and enable ~nightly~ (~rustup install nightly; rustup default nightly~).

To run from the repository directly, clone and ~cargo run --release -- <pa-bin flags>~.

For an independent install,
#+begin_src shell
