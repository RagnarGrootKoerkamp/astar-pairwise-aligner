...
The most important flags are given by ~cargo run -- -h~ from anywhere in the repository. Use
~--help~ for more explanation and more flags/features not used in the paper.
#+begin_src shell
USAGE:
...
ALGORITHM:
    -a, --algorithm <ALGORITHM>    [default: a-star] [possible values: nw, nw-simd, a-star]
...
VISUALIZER:
    -v, --visualize [<WHEN>...]    Run the interactive visualizer. See --help for controls. [default: all] [possible
                                   values: none, last, all, layers]
...
#+end_src
Note that input can either from a file (~--input~) or randomly
