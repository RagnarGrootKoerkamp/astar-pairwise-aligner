...
class Input:
    ...
    def pattern():
        return 'evals/input/x{cnt}-n{n}-e{e}.seq'
    def name(self):
        return Path(f'x{self.cnt}-n{self.n}-e{self.e}')
    def path(self):
        return Path(f'evals/input/{self.name()}.seq')
...
class Run:
    ...
    def pattern_with_params():
        alg = 'pa'
        return f'evals/runs/{Input.name_pattern()}-A{alg}-m{{m}}-k{{k}}-pf{{pf}}.bench'
    def pattern(alg='pa'):
        return f'evals/runs/{Input.name_pattern()}-A{alg}.bench'
    def bench_path_with_params(self):
        return Path(f'evals/runs/{self.input.name()}-A{self.alg}-m{self.m}-k{self.k}-pf{self.pf}.bench')
    def bench_path(self):
        return Path(f'evals/runs/{self.input.name()}-A{self.alg}.bench')
    def stats_path(self):
        assert self.alg == "pa"
...
rule tools_table:
    ...
    output: 'evals/table/tools_N{N}.tsv'
    run:
